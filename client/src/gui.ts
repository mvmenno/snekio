...
export class Gui {
    ...
    private GuiShadersEnabled: GUI.AdvancedDynamicTexture;
    private nickName: string;

    private validPlayState: boolean = false;

    public readonly shadersEnabled: boolean = true;
    ...
    removeMainMenuElements() {
        ...
        this.GuiShadersEnabled.dispose();
    }
    triggerPlayState() {
    ...
    createMainMenuToggleShaderButton() {
        var advancedTexture = GUI.AdvancedDynamicTexture.CreateFullscreenUI("UI");
        ...
        var shaderButton = GUI.Button.CreateSimpleButton("shader-button", "Toggle shaders: "+this.shadersEnabled ? "on" : "off");
        ...
        var shadersEnabled = this.shadersEnabled;

        shaderButton.onPointerClickObservable.add(function (e) {
            this.shadersEnabled = !this.shadersEnabled;

            shaderButton.color = this.shadersEnabled ? "#00ff00" : "#ff0000";

            if(this.shadersEnabled){
                shaderButton.textBlock.text = "on";
            }else{
                shaderButton.textBlock.text = "off";
            }
        }.bind(this));

        advancedTexture.addControl(textFieldTitle);
        advancedTexture.addControl(shaderButton);

        this.GuiShadersEnabled = advancedTexture;
    }
